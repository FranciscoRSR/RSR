...
function saveAllParticipants() {
    ...
    event.participants.forEach((participant, participantIndex) => {
        ...
        event.days.forEach((day, dayIndex) => {
            ...
            // Ensure driven_per_day[dayIndex] is properly initialized
    ...
    // Filter out participants with no cars assigned
    ...
    event.participants.forEach(participant => {
        ...
        Object.entries(circuitGroups).forEach(([circuitName, group]) => {
            ...
            group.days.forEach(day => {
                ...
                if (day.finished) {
                    // Day is finished - show what was driven
                    carsForDay.forEach((carPlate, carIdx) => {
                        const car = cars.find(c => c.license_plate === carPlate);
                        if (car) {
                            const packageType = packagesForDay[carIdx] || 'basic';
                            const driven = drivenForDay[carIdx] || 0;
                            dayContent += `Finished ${driven} ${group.unit} (${packageType})<br>`;
                        }
                    });
                } else {
                    ...
                    if (balanceTrack < 1) {
                        ...
                        // Calculate remaining laps/kms for this circuit
                        ...
                        carsForDay.forEach((carPlate, carIdx) => {
                            ...
                            if (car) {
                                ...
                                const lapsDriven = TotalLapsModel(participant, event, circuitName, carModel, packageType);
                                
                                totalAvailable += lapsAvailable;
                                totalDriven += lapsDriven;
                            }
                        });
                        
                        const totalRemaining = totalAvailable - totalDriven;
                        ...
                        if (remainingPerDay >= 1) {
                            dayContent = `Remaining ${remainingPerDay} ${group.unit}`;
                        } else {
                            dayContent = `0 (No credit)`;
                        }
                    }
